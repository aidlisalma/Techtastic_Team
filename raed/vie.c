...
void Init_Vie(Vie *v) //initialisation vie_score
{
	v->vie=5;       // vie de l'hero au debut de jeu
	v->survie=true; //l'hero est en vie 
    v->score=100;  // inisialisation de score au 100
}
...
void Calcul_Vie(Vie *v) 
{
	...
	if(v->vie==0)
	{
		v->survie=false; // l'hero est mort
	}
}
...
void Display_Vie(Vie *v,SDL_Surface*ecran,SDL_Rect position)
{
        ...
        /*-- Declaration des surfaces de vie--*/
SDL_Surface *vie0;
SDL_Surface *vie1;
SDL_Surface *vie2;
SDL_Surface *vie3;
...
SDL_Surface *temp,*score;

/*-------configuaration de temps de jeu--------*/
int tempsdebut=0;
SDL_Rect positiontemp;
positiontemp.x=480;
positiontemp.y=0;
char texte[40],scor[20];
sprintf(texte, "**TEMPS DE JEU:%dsec**", (SDL_GetTicks() )/1000);
temp = TTF_RenderText_Blended(police_score, texte, couleurNoire);
SDL_BlitSurface(temp, NULL, ecran, &positiontemp);
