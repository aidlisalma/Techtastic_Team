...
void Animation_Ennemy(Ennemy *e) //(animation entité secondaire) RAED
{
	e->Animation++; 	//auto animation
	if(e->Animation==4) //jouer en boucle
	{
		e->Animation=1;
		if(e->Direction)
		{
	...
	if(e->Direction)	//verification de deplacement
	{
		e->Position.x+=70;	//distance de deplacement
	}
	else
